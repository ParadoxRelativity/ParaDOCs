...
import type { Permission, Role } from '@paradocs/shared';
import { query } from '../db/pool.js';
import { forbidden, notFound, unauthorized } from './http.js';

/**
 * Who may reach a folder, document or channel.
 *
...
/** What someone may do with a document, or null when they may not see it. */
...
/** What someone may do in a folder, or null when they may not see it. */
export async function folderAccess(userId: string, folderId: string): Promise<ResourceAccess | null> {
  if (!UUID.test(folderId)) return null;
  const { rows } = await query<{ workspace_id: string; role: Role | null; level: number | null }>(
    `SELECT f.workspace_id, m.role, ${folderLevelSql('$2', 'm.role', 'f.id')} AS level
       FROM folders f
 ...
 * One they may not see, or one outside `workspaceId` when that is given, is
 * not found rather than forbidden, so a folder id reveals nothing.
 */
export async function assertFolderAccess(
  ...
  if (!access || (workspaceId && access.workspaceId !== workspaceId)) throw notFound('Folder not found');
 ...
 *
 * `item` is the document or folder being moved: its own setting, and the
